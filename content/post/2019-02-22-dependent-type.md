...
在定义中放入`(same 4)`也会得到类似的错误提示。如果套用传统编程语言对类型的定位来理解`=`类型的话，会发现很难说得通。因为`=`类型并不像前面的`Nat`或`Vec`类型，它看起来不像是一类值的集合。所以我们应该换一个角度来看待`=`类型，可以把它看作一个**陈述或命题**。所以`(= Nat (+ 1 1) 2)`可以读作“`(+ 1 1)`和`2`是两个相等的自然数”。这时`=`类型的值就变成它所代表的那个命题的一个**证明**。那么为什么`(same 2)`就是`(= Nat (+ 1 1) 2)`的一个证明呢？因为我们举证出了一个值`2`，它既等于`(+ 1 1)`也等于`2`。同理，不存在`(= Nat 3 4)`类型的值也是因为我们没法找到一个可以证明`3`等于`4`的证据。像这种通过举证来证明的方法在数学中被叫作[构造性证明（Constructive Proof）](https://en.wikipedia.org/wiki/Constructive_proof)[^8]。

有了这个看待类型的全新角度后，其实我们可以把任意类型都看作一个命题。比如`Atom`就是一个命题，而任何一个`Atom`类型的值都是它的证明，只不过这个命题的证明对我们来说没有多大的用处。而让我们感兴趣的一类命题是，把`Π`表达式与`=`类型结合所得到的命题。因为`=`类型也是依赖类型，所以它是可以放在`Π`表达式里，此时可以把`Π`看作全称量词，整个表达式就变成了一个全称命题。比如下面这个声明：
...
可以把它读作，“对于任意一个自然数`n`，`(+ 1 n)`与`(add1 n)`的值相等”。这样`1+=add1`就可以看作是对命题的陈述。那么要如何证明这种全称命题？其实就是实现一个由`Π`表达式声明的函数，这个函数返回一个`=`类型的值。
...
这里`TODO`代表的是`=`类型的返回值，因为`=`类型的 constructor 只有`same`，所以`TODO`是一个`same`表达式，现在的问题是应该在`same`里放入什么样的表达式。回忆一下`+`函数的定义：
...
可以看出来`(+ 1 n)`最后结果是`(add1 n)`，所以应该用`(same (add1 n))`来替换`TODO`。其实如果看一下解释器输出的`TODO`类型提示，已经可以知道`same`里应该放入`(add1 n)`：
...
[^8]: [《计算进化史》](https://book.douban.com/subject/26975991/)这本书里有比较全面的关于这方面的讨论。