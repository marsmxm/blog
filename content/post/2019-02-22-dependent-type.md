...
有了这个看待类型的全新角度后，其实我们可以把任意类型都看作一个命题。比如`Atom`就是一个命题，而任何一个`Atom`类型的值都是它的证明，只不过这个命题的证明对我们来说没有多大的用处。因为`=`类型也是依赖类型，所以它可以放在`Π`表达式里，此时可以把`Π`看作全称量词，整个表达式就变成了一个全称命题。比如下面这个声明：
...
可以把它读作，“对于任意一个自然数`n`，`(+ 1 n)`与`(add1 n)`的值相等”。
