...
因为`Even`接受一个自然数类型`n`，返回一个关于`n`的定理（`n`是偶数），而定理即类型，所以`Even`也是一个生成类型的函数，需要在声明中把返回值类型写作`U`，而在函数定义中才返回一个`Σ`表达式。现在就可以把“0 是偶数”这个命题声明为：
...
我们知道把任意一个偶数加 2 得到的仍然是偶数，我们可以试着用 Pie 来证明这个定理。首先可以写出它的类型声明：
